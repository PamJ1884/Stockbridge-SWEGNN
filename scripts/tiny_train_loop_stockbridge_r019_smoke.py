...
def parse_args() -> argparse.Namespace:
    """Parse command-line options for the tiny training loop."""
    parser = argparse.ArgumentParser(
        description="Run a tiny SWE-GNN training loop on the R019 graph."
    )
    parser.add_argument(
        "--device",
    ...
    parser.add_argument(
        "--allow-cpu",
        action="store_true",
        help="Allow the full-graph tiny training loop to run on CPU.",
    )
...
def select_device(requested_device: str, allow_cpu: bool) -> torch.device:
    ...
    if device_type == "cpu" and not allow_cpu:
        raise RuntimeError(
            "CUDA is not available. Refusing to run full-graph tiny training loop "
            "on CPU unless --allow-cpu is set."
        )
...
def build_one_step_sample(
    ...
    """Build an in-memory model sample for timestep t to t+1."""
    H_t = graph.H[:, t]
    Qmag_t = graph.Qmag[:, t]
    target = torch.stack(
    ...
    sample.rainfall_target = graph.rainfall_global[t + 1].float()
...
def evaluate_model(
    ...
    """Evaluate mean, minimum, and maximum one-step losses."""
    losses = []
    model.eval()

    with torch.no_grad():
        for t in time_indices:
            sample = build_one_step_sample(graph, slope_x, slope_y, edge_attr, t)
...
def main() -> None:
    ...
    print("=== Stockbridge SWE-GNN tiny training loop smoke test ===")
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        for t in epoch_time_indices:
            ...
            input_h_max = sample.x[:, 3].max().item()
            target_h_max = sample.y[:, 0].max().item()
            target_qmag_max = sample.y[:, 1].max().item()
            rainfall_input = sample.rainfall_input.item()
            rainfall_target = sample.rainfall_target.item()
            print(
                ...
                f"rainfall_input={rainfall_input:.8g} "
                f"rainfall_target={rainfall_target:.8g}"
            )
    ...
    print("\nStockbridge SWE-GNN tiny training loop smoke test passed.")
