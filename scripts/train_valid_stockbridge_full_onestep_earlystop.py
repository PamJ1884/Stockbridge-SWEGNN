...
VALID_EVENT_METRICS_FIELDNAMES = [
    "event_id",
...
def parse_args() -> argparse.Namespace:
    """Parse command-line options for milestone 14B-1."""
    parser = argparse.ArgumentParser(
        description=(
            "Run full-dataset one-step teacher-forced SWE-GNN training with "
    ...
    parser.add_argument(
        "--allow-cpu",
        action="store_true",
        help="Allow full-dataset one-step training to run on CPU.",
    )
...
def select_device(requested_device: str, allow_cpu: bool) -> torch.device:
    ...
    if device_type == "cpu" and not allow_cpu:
        raise RuntimeError(
            "CUDA is not available. Refusing to run full-graph milestone 14B-1 "
...
def evaluate_validation(
    ...
    """Run teacher-forced validation and return aggregate and event metrics."""
    losses = []
    H_RMSE_values = []
    H_MAE_values = []
    Qmag_RMSE_values = []
    ...
    with torch.inference_mode():
        for event in prepared_valid_events:
            ...
            event_H_RMSE_values = []
            event_H_MAE_values = []
            event_Qmag_RMSE_values = []
            event_Qmag_MAE_values = []
            event_combined_MSE_values = []
            for t in time_indices:
                ...
                combined_MSE_value = combined_MSE.item()
                losses.append(loss.item())
                H_RMSE_values.append(H_RMSE)
                H_MAE_values.append(H_MAE)
                Qmag_RMSE_values.append(Qmag_RMSE)
                Qmag_MAE_values.append(Qmag_MAE)
                combined_MSE_values.append(combined_MSE_value)
                event_H_RMSE_values.append(H_RMSE)
                event_H_MAE_values.append(H_MAE)
                event_Qmag_RMSE_values.append(Qmag_RMSE)
                event_Qmag_MAE_values.append(Qmag_MAE)
                event_combined_MSE_values.append(combined_MSE_value)
            ...
            event_H_peak_ratios.append(H_peak_ratio)
            event_metrics.append(
                {
                    "event_id": event["event_id"],
                    "max_true_H_over_eval": max_true_H_over_eval,
                    "max_pred_H_over_eval": max_pred_H_over_eval,
                    "H_peak_ratio": H_peak_ratio,
                    "mean_H_RMSE": (
                        sum(event_H_RMSE_values) / len(event_H_RMSE_values)
                    ),
                    "mean_H_MAE": (
                        sum(event_H_MAE_values) / len(event_H_MAE_values)
                    ),
                    "mean_Qmag_RMSE": (
                        sum(event_Qmag_RMSE_values)
                        / len(event_Qmag_RMSE_values)
                    ),
                    "mean_Qmag_MAE": (
                        sum(event_Qmag_MAE_values)
                        / len(event_Qmag_MAE_values)
                    ),
                    "mean_combined_MSE": (
                        sum(event_combined_MSE_values)
                        / len(event_combined_MSE_values)
                    ),
                }
            )
    ...
    for event_summary in event_metrics:
        for name, value in event_summary.items():
            if name != "event_id" and value is not None:
                if not math.isfinite(float(value)):
                    raise ValueError(
                        "Non-finite validation event metric "
                        f"{event_summary['event_id']} {name}={value}"
                    )
...
def train_one_epoch(
    ...
    """Run one epoch and optionally track pre-clipping gradient norms."""
    ...
    for event_index, t in epoch_pairs:
        ...
        if grad_clip_norm > 0.0:
            ...
            if not torch.isfinite(gradient_norm).item():
                raise ValueError(
                    f"Non-finite gradient norm during {context}: "
    ...
    return (*summarize_losses(losses), mean_gradient_norm)
...
def build_checkpoint(
    ...
    """Build a self-describing Milestone 14B-1 checkpoint dictionary."""
    return {
        "epoch": epoch,
...
def write_valid_event_metrics(
    output_path: Path,
    event_metrics: list[dict[str, float | str | None]],
) -> None:
    """Write one naturally ordered row per validation event."""
    if not event_metrics:
        raise ValueError("Cannot write empty validation event metrics")
    with output_path.open("w", newline="") as output_file:
        writer = csv.DictWriter(
            output_file,
            fieldnames=VALID_EVENT_METRICS_FIELDNAMES,
        )
        writer.writeheader()
...
def main() -> None:
    ...
    history_csv = output_dir / "milestone14b1_history.csv"
    best_checkpoint_path = output_dir / "milestone14b1_best_checkpoint.pt"
    final_checkpoint_path = output_dir / "milestone14b1_final_checkpoint.pt"
    summary_json = output_dir / "milestone14b1_summary.json"
    best_valid_event_metrics_csv = (
        output_dir / "milestone14b1_best_valid_event_metrics.csv"
    )
    final_valid_event_metrics_csv = (
        output_dir / "milestone14b1_final_valid_event_metrics.csv"
    )
    output_paths = [
        history_csv,
    ...
    if existing_outputs and not args.overwrite:
        ...
        raise FileExistsError(
            "Refusing to overwrite existing milestone 14B-1 outputs without "
            f"--overwrite:\n{formatted}"
        )
    ...
    # Inspect test IDs only for split-leakage validation. Test tensors are never
    # loaded, prepared, passed to the model, or included in output metrics.
    ...
    if "R019" not in full_test_event_ids:
        raise ValueError("R019 must be present in the test event IDs")
    overlap = sorted(
    ...
    print("=== Stockbridge SWE-GNN milestone 14B-1 one-step training ===")
    ...
    initial_valid_summary, _ = evaluate_validation(
        model,
        static_device_graph,
    ...
    with history_csv.open("w", newline="") as history_file:
        ...
        for epoch in range(1, args.epochs + 1):
            ...
            valid_summary, valid_event_metrics = evaluate_validation(
                model,
                static_device_graph,
                prepared_valid_events,
                time_indices,
            ...
            is_best = valid_loss_mean < best_valid_loss - args.min_delta
            if is_best:
                best_valid_loss = valid_loss_mean
                best_epoch = epoch
                best_valid_summary = valid_summary.copy()
                epochs_since_improvement = 0
                torch.save(
                    build_checkpoint(
                        epoch,
            ...
            if train_loss_summary[3] is not None:
                print(
                    "mean gradient norm before clipping: "
                    f"{train_loss_summary[3]:.8g}"
                )
            print(f"best_valid_loss_so_far: {best_valid_loss:.8g}")
    ...
    final_valid_summary, final_valid_event_metrics = evaluate_validation(
        model,
        static_device_graph,
        prepared_valid_events,
        time_indices,
    ...
    write_valid_event_metrics(
        final_valid_event_metrics_csv,
        final_valid_event_metrics,
    )
    if best_epoch <= 0 or best_valid_summary is None:
    ...
    summary = {
        ...
        "best_epoch": best_epoch,
        "best_valid_loss": best_valid_loss,
        "best_valid_summary": best_valid_summary,
        ...
        "best_valid_event_metrics_csv": str(best_valid_event_metrics_csv),
        "final_valid_event_metrics_csv": str(final_valid_event_metrics_csv),
        "device": str(device),
    ...
    print()
    print(
        "Stockbridge SWE-GNN milestone 14B-1 one-step early stopping "
        "training passed."
    )
