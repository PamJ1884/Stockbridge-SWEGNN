...
def parse_args() -> argparse.Namespace:
    """Parse command-line options for the train-valid smoke test."""
    parser = argparse.ArgumentParser(
        description="Run a tiny rainfall-forced SWE-GNN train-valid smoke test."
    )
    parser.add_argument(
        "--device",
    ...
    parser.add_argument(
        "--allow-cpu",
        action="store_true",
        help="Allow the full-graph train-valid smoke test to run on CPU.",
    )
...
def select_device(requested_device: str, allow_cpu: bool) -> torch.device:
    ...
    if device_type == "cpu" and not allow_cpu:
        raise RuntimeError(
            "CUDA is not available. Refusing to run full-graph train-valid "
            "rainfall tiny smoke test on CPU unless --allow-cpu is set."
        )
...
def evaluate_model(
    ...
    """Evaluate all selected event/timestep pairs without updating the model."""
    losses = []
    model.eval()
...
def print_loss_summary(
    label: str,
    loss_summary: tuple[float, float, float],
) -> None:
    """Print mean, minimum, and maximum loss with a shared format."""
    mean_loss, min_loss, max_loss = loss_summary
    print(
        f"{label}: mean_loss={mean_loss:.8g} "
        f"min_loss={min_loss:.8g} max_loss={max_loss:.8g}"
    )
...
def main() -> None:
    ...
    print("=== Stockbridge SWE-GNN train-valid rainfall tiny smoke test ===")
    ...
    print("Train event IDs:", [graph.event_id for graph in train_graphs])
    print("Valid event IDs:", [graph.event_id for graph in valid_graphs])
    ...
    print("\nInitial evaluation:")
    print_loss_summary("Initial train evaluation", initial_train)
    print_loss_summary("Initial valid evaluation", initial_valid)
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        for event_index, t in epoch_train_pairs:
            graph, slope_x, slope_y, edge_attr = prepared_train_events[event_index]
            sample = build_one_step_sample(
                graph,
                slope_x,
                slope_y,
        ...
        epoch_update_summary = summarize_losses(epoch_losses)
        epoch_elapsed = time.perf_counter() - epoch_start
        epoch_train_evaluation = evaluate_model(
            model,
            prepared_train_events,
            train_pairs,
            args.rainfall_scale,
        ...
        epoch_valid_evaluation = evaluate_model(
            model,
            prepared_valid_events,
            valid_pairs,
            args.rainfall_scale,
        ...
        print(f"\nEpoch {epoch} summary:")
        print_loss_summary("Train update losses", epoch_update_summary)
        print(f"elapsed_seconds={epoch_elapsed:.2f}")
        print_loss_summary("Train evaluation", epoch_train_evaluation)
        print_loss_summary("Valid evaluation", epoch_valid_evaluation)
    ...
    print("\nFinal evaluation:")
    print_loss_summary("Final train evaluation", final_train)
    print_loss_summary("Final valid evaluation", final_valid)
    ...
    print()
    print("Stockbridge SWE-GNN train-valid rainfall tiny smoke test passed.")
